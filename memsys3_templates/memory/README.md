...
Evitar que els agents llegeixin desenes d'arxius de codi per entendre el projecte. En lloc d'això, carreguen un únic arxiu `context.yaml` (màxim 2000 línies) amb tota la informació crítica.
...
# 4. Compila el context quan sigui necessari
...
Escriu en aquests arxius durant el desenvolupament:
...
- ✅ Notes que `context.yaml` està obsolet
...
- Linkeja ADRs des de sessions si és necessari

**❌ Evitar:**
- Documentar cada petit canvi
- Duplicar informació entre arxius
- Descripcions vagues ("millorat sistema")
- Massa detall tècnic ("canviat padding 10px→12px")

### Per Compilar

**✅ Fer:**
- Executa CA regularment (setmanalment o després de sessions grans)
...
- Crea nou arxiu buit per continuar
- **No es perden dades**, queden a `sessions_1.yaml`, `sessions_2.yaml`, etc.

**Context Agent llegeix tots els arxius** (`sessions.yaml` + `sessions_*.yaml`) fins que total >150K tokens, llavors arxiva irrellevants a `history/`.

### Arxivat Intel·ligent (>150K tokens)

Si el CA detecta >150K tokens totals:
- Mou ADRs/sessions irrellevants a `memory/history/`
- `history/` **NO es llegeix** → estalvi real
- Dades preservades, recuperables si és necessari

### Periodicitat

- **Context Agent**: Executa després de sessions importants
- **Rotació**: Automàtica quan >1800 línies
- **Arxivat**: Automàtic del CA quan >150K tokens
...
2. Al final, escrius a `memsys3/memory/full/sessions.yaml`:
   ...
   ```
3. Si has decidit alguna cosa arquitectònica, afegeix ADR a `memsys3/memory/full/adr.yaml`
...
R: Potser necessita codi específic. El CA només dóna visió general, no substitueix llegir codi quan és necessari.

**P: memsys3/memory/full/sessions.yaml és immens**
R: Normal després de mesos. El CA només pren l'última sessió per `memsys3/memory/context.yaml`. Si molesta, pots arxivar sessions antigues.
