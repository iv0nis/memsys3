...
Registrar què s'ha fet durant aquesta sessió perquè el proper DevAgent tingui context complet.

## Workflow

### 1. Recopilar Evidències Objectives

Recopila evidències del que s'ha fet:

```bash
# Git (si disponible)
git status && git diff --stat && git log --oneline -5

# Arxius modificats (ajusta -mmin segons durada sessió)
...
- **Propers Passos**: Què queda pendent
...
**Rotació = Còpia segura + Verificació + Nou arxiu buit**
...
- **IMPORTANT - Gotchas**: Si has trobat errors crítics, warnings o traps:
  ...
  - Incloure: `tipus`, `problema`, `solucio`, `criticitat` (alta|media|baja)
  - Només gotchas rellevants (errors que trenquen, contra-intuïtius, recurrents)
...
3. Linkar ADR des de sessió (camp `adr_relacionada`)
...
- `pendientes_prioritarios`: Actualitzar segons propers passos

### 5. Informar l'Usuari

Resum breu del que s'ha documentat:
...
Propers passos: [Top 2-3 tasques pendents]
```
