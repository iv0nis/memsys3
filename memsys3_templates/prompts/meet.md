...
```bash
FILE="memsys3/docs/meets/DDMMYY_N.md"
for i in $(seq 1 20); do
  if grep -q "^## Agent [DESTINO]" "$FILE"; then
    awk '/^## Agent [DESTINO]/,/^---/' "$FILE"
    exit 0
  fi
  sleep 15
done
...
Adaptar `[DESTINO]` al nombre del agente que esperas.
...
```yaml
highlights:
  - "Reunión [tipo] con [Agente] sobre [tema] → memsys3/docs/meets/DDMMYY_N.md"
  - "[Resultado principal]"
```
