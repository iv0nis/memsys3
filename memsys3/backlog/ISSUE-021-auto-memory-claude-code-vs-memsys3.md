...
**Origen:** Conversación con agente WORKSPACE_Y (Claude Code con auto-memory activa)
...
El agente usó "***" como valor por defecto silencioso al rellenar un campo "lugar" porque lo tenía guardado en MEMORY.md como "Ubicación: ***, ***" **sin contexto de para qué sirve ni cuándo aplicarlo**. Aplicó el dato como default en vez de preguntar.
...
**⚠️ Hallazgo posterior (2026-04-20, segunda conversación con WORKSPACE_Y):**
...
Esto invalida parcialmente Opción A como solución limpia. El workaround aplicado en WORKSPACE_Y fue:
...
Caso real WORKSPACE_Y: 9 archivos acumulados (4 feedback, 4 project, 1 user) todos con info duplicable en memsys3 (sessions.yaml, project-status.yaml, prompts, o el propuesto MEMORY_memsys3.yaml).
...
**Parte 1 — detección:**
- Usuario detecta que agente usó "***" como default silencioso
- Agente reconoce: lo tiene en MEMORY.md como "Ubicación: ***, ***" sin contexto de uso
...
- Descubrimiento del alcance real: además de MEMORY.md, el sistema auto-memory crea archivos individuales:
  ...
  - `project_*.md` (hechos de proyecto): ***_2026_04_16, paddely, pec_programacion_entregada, root_cleanup
  - `user_*.md` (hechos de usuario): ***
- 9 archivos acumulados en un solo proyecto, todos con info duplicable en memsys3

Ruta MEMORY.md en el caso observado:
`***/.claude/projects/-home-ivonis-WORKSPACE_Y/memory/MEMORY.md`
